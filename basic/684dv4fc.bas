100 ' #######################################################################
110 '
120 '    Distance variation of LF resultant field strength of 1 $ 2 hop
130 '    sky-waves and ground-wave as well as its phase delay relative
140 '    to ground-wave
150 '
160 '                          by N. Wakai
170 '
180 '                                              (684DV4FC.BAS)  Oct. 2005
190 ' #######################################################################
200 '
210 '  This program is applicable for calculating the distance variation of
220 '  resultant field strength of 1 & 2 hop sky-wave and the ground-wave
230 '  at frequencies between 40 and 500 kHz over ground ranges from 100 to
240 '  4000 km as well as phase delay of resultant relative to ground-wave.
250 '                                       (parabolic D/E layer model)
260 '
270 CLS
280 #INCLUDE "windows.bi"
...
560 '
570 OPEN "c:\lfdata.dat" AS #1 :CLOSE #1 :KILL "c:\lfdata.dat"
580 OPEN "c:\lfdata.dat" FOR OUTPUT AS #1
590 J=1
600 FOR I=DMIN TO DMAX STEP DSTP
610 DIST(J)=I :CA=DIST(J)/6367
620 '
630 '  reflection point coordinate and height
...
740 FOR K=1 TO 2
750 HOP=K
760 '  ray path length, elevation and incidence angles and delay
...
950 GOSUB *INTFF
960 '
970 '  transmitting antenna factor
980 PSI=PSI*RTD
990 RESTORE 3970
1000 FOR M=1 TO 13
1010 READ PS(M)
...
1260 FR=ANT
1270 '
1280 ' ionospheric reflection coefficient
1290 RESTORE 4240
1300 FOR M=1 TO 11
1310 READ CSX(M)
1320 NEXT M
...
1460 GOSUB *INTRC
1470 '
1480 '  reflection coefficient of ground
1490 PSI=PSI*DTR
...
1750 NEXT K
1760 '
1770 '  ground-wave and resultant field strength
1780 GOSUB *GWAVE
...
2020 OPEN "c:\lfdata.dat" FOR INPUT AS #1
...
2220 C=5
2230 *LFV :IF EOF(1) THEN GOTO *FIN
...
2310 GOTO *LFV
2320 *LFVT :IF EOF(1) THEN GOTO *FIN
...
2440 END
2450 '
2460 '  reflection height subroutine
2470 *REFH
...
2860 RETURN
2870 '
2880 '  focusing factor subroutine
2890 *INTFF
2900 FOR N=1 TO 16
...
3180 RETURN
3190 '
3200 '  antenna factor subroutine
3210 *INTAF
3220 FOR N=1 TO 13
...
3500 RETURN
3510 '
3520 '  ionospheric reflection coefficient subroutine
3530 *INTRC
3540 FOR N=1 TO 15
...
4620 '
4630 ' Ground Wave field strength based on Rec. P.368
4640 *GWAVE
4650 RESTORE 5260
...
5230 GOTO 4900
5240 '
5250 'ground wave field strength data
